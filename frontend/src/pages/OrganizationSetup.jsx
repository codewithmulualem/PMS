...
const TABS = [
  { key: "structure", label: "የድርጅት መዋቅር" },
  { key: "cycles", label: "ዑደቶች" },
  { key: "weights", label: "የነጥብ ክብደቶች" },
  { key: "competencies", label: "ብቃቶች" },
];
const CATEGORY_LABELS = { technical: "ቴክኒካዊ", behavioral: "ባህሪ", leadership: "አመራር" };
...
export default function OrganizationSetup() {
  ...
  return (
    <div>
      <Topbar subtitle="የድርጅት መዋቅር፣ የአፈጻጸም ዑደቶች፣ የነጥብ ቅንብር እና የብቃት ማዕቀፍ" />
...
function CyclesTab({ cycles, onChanged, toast }) {
  ...
  async function add(e) {
    ...
    toast.push(`የ“${form.name}” ዑደት ተፈጥሯል`, "success");
  ...
  return (
    <div className="grid grid-2">
      <div className="card">
        <div className="card-title">አዲስ የአፈጻጸም ዑደት</div>
        <form onSubmit={add}>
          <div className="field">
            <label>የዑደት ስም</label>
            <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="ለምሳሌ፦ የ2017 ዓ.ም. የመጀመሪያ ግማሽ" />
          </div>
          ...
          <div className="field">
            ...
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
              ...
              <option value="closed">ተዘግቷል</option>
            </select>
          </div>
          <button className="btn btn-primary" type="submit"><Icons.plus size={15} /> ዑደት ፍጠር</button>
        </form>
      </div>
      <div className="card">
        <div className="card-title">ዑደቶች</div>
        {cycles.length === 0 ? (
          <div className="empty-state">እስካሁን ዑደት አልተፈጠረም።</div>
        ) : (
          cycles.map((c) => (
            <div key={c.id} className="flex-between" style={{ padding: "10px 2px", borderBottom: "1px solid #f0f2f7" }}>
              ...
              {c.status === "active"
                ...
                : c.status === "closed"
                  ? <span className="chip chip-neutral">ተዘግቷል</span>
                  : <span className="chip chip-indigo">ታቅዷል</span>}
            </div>
          ))
        )}
      </div>
...
function WeightsTab({ cycles, toast }) {
  ...
  const FIELDS = [
    ["kpi_weight", "KPIዎች"],
    ["goal_weight", "ግቦች"],
    ["competency_weight", "ብቃቶች"],
    ["behavior_weight", "ባህሪ"],
    ["program_weight", "ፕሮግራሞች"],
  ];
  ...
  async function save() {
    ...
    try {
      ...
      toast.push("የነጥብ ክብደቶች ተሻሽለዋል — አዲስ ስሌቶች እነዚህን ይጠቀማሉ", "success");
    } catch (err) {
  ...
  return (
    <div className="grid grid-2">
      <div className="card">
        <div className="card-title">ጠቅላላ ነጥብ = <EpistemicTag kind="calc" /></div>
        <div className="mono" style={{ fontSize: 12.5, background: "#f5f6fb", padding: "12px 14px", borderRadius: 10, lineHeight: 1.9, marginBottom: 16 }}>
          የKPI ነጥብ × የKPI ክብደት<br />
          + የግብ ስኬት × የግብ ክብደት<br />
          + የብቃት ነጥብ × የብቃት ክብደት<br />
          + ባህሪ × የባህሪ ክብደት<br />
          + ፕሮግራሞች × የፕሮግራም ክብደት
        </div>
        <div className="text-faint" style={{ fontSize: 12.5, lineHeight: 1.6 }}>
          ክብደቶች በየዑደቱ ሊዋቀሩ ይችላሉ እና ካልተዋቀሩ የድርጅቱን ነባሪ ይጠቀማሉ። ስሌቱ መረጃ ባላቸው ክፍሎች ላይ ብቻ እንደገና ስለሚመዝን ከፊል ግምገማ ሌሎችን አያዛባም።
        </div>
      </div>

      <div className="card">
        <div className="card-title">ክብደቶችን ያዋቅሩ</div>
        <div className="field">
          <label>ዑደት</label>
        ...
        {!weights ? (
          <div className="empty-state">ክብደቶች በመጫን ላይ…</div>
        ) : (
          <>
            ...
            <div className="flex-between">
              ...
              <button className="btn btn-primary" onClick={save} disabled={saving || Math.abs(total - 100) > 0.001}>
                {saving ? "በማስቀመጥ ላይ…" : "ክብደቶችን አስቀምጥ"}
              </button>
            </div>
            {Math.abs(total - 100) > 0.001 && (
              <div className="text-faint" style={{ fontSize: 11.5, marginTop: 6 }}>ከማስቀመጥዎ በፊት ክብደቶች ድምር 100% መሆን አለበት።</div>
            )}
          </>
        )}
      </div>
...
function CompetenciesTab({ competencies, onChanged, toast }) {
  ...
  async function add(e) {
    ...
    toast.push(`ብቃት “${form.name}” ተጨምሯል`, "success");
    setForm({ name: "", category: (cats[0] && cats[0].code) || "technical", definition: "" });
    onChanged();
  }

  const byCat = (cat) => competencies.filter((c) => c.category === cat);
  const defaultCats = [{ code: "technical", label: "ቴክኒካዊ" }, { code: "behavioral", label: "ባህሪ" }, { code: "leadership", label: "አመራር" }];
  const frameworkCats = cats.length ? cats : defaultCats;

  return (
    <div className="grid grid-2">
      <div className="card">
        <div className="card-title">ብቃት ጨምር</div>
        <form onSubmit={add}>
          <div className="field">
            <label>ስም</label>
            <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="ለምሳሌ፦ የመረጃ ትንተና" />
          </div>
          <div className="field">
            <label>ምድብ</label>
            <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
              {frameworkCats.map((c) => (
                <option key={c.code} value={c.code}>{CATEGORY_LABELS[c.code] || c.label}</option>
              ))}
            </select>
          </div>
          <div className="field">
          ...
          <button className="btn btn-primary" type="submit"><Icons.plus size={15} /> ብቃት ጨምር</button>
        </form>
      </div>
      <div className="card">
        <div className="card-title">የብቃት ማዕቀፍ</div>
        {frameworkCats.map((cat) => (
          <div key={cat.code} style={{ marginBottom: 14 }}>
            <div className="flex-between" style={{ marginBottom: 4 }}>
              <span className="cell-strong" style={{ textTransform: "capitalize" }}>{CATEGORY_LABELS[cat.code] || cat.label}</span>
        ...
        {competencies.length === 0 && <div className="empty-state">እስካሁን ብቃት አልተገለጸም።</div>}
      </div>
    </div>
  );
}
